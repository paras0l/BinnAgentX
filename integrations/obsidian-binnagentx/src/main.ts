...
const SIMPLE_VOCABULARY_TEMPLATE =
  '---\nbinnagent_schema: "learning-context/v1"\nbinnagent_kind: "vocabulary"\nmeaning: ""\nstatus: learning\ncreated: "{{date}}"\ntags:\n  - binnagent\n  - vocabulary\n  - flashcards/binnagentx/vocabulary\n---\n\n# {{title}}\n\n## 核心含义\n\n## 发音\n\n## 常用搭配\n\n## 原句与语境\n\n## 我的例句\n\n## 易混淆点\n\n## 闪卡\n\n> 先填写“核心含义”。下面的卡片会复用这份内容，不需要重复维护释义。\n\n{{title}} 的核心含义是什么？::![[#核心含义]]\n\n## 关联\n- [[BinnAgentX/01-Vocabulary/00-Dashboard|词汇 Dashboard]]\n- [[BinnAgentX/Spaced Repetition 使用指南|Spaced Repetition 使用指南]]\n';

const VOCABULARY_TEMPLATE =
  '---\nbinnagent_schema: "learning-context/v1"\nbinnagent_kind: "vocabulary"\nmeaning: ""\nstatus: learning\ncreated: "{{date}}"\ntags:\n  - binnagent\n  - vocabulary\n  - flashcards/binnagentx/vocabulary\n---\n\n# {{title}}\n\n## 核心含义\n\n## 发音\n\n## 常用搭配\n\n## 原句与语境\n\n## 我的例句\n\n## 易混淆点\n\n## 闪卡\n\n> [!binn-prompt] 单词回忆\n> # {{title}}\n> 先说出核心含义，再回忆一个搭配或例句。\n?\n> [!binn-meaning] 核心释义\n> ![[#核心含义]]\n---\n> [!binn-sound] 发音\n> ![[#发音]]\n---\n> [!binn-collocations] 常用搭配\n> ![[#常用搭配]]\n---\n> [!binn-example] 我的例句\n> ![[#我的例句]]\n---\n> [!binn-contrast] 易混淆点\n> ![[#易混淆点]]\n\n## 关联\n- [[BinnAgentX/01-Vocabulary/00-Dashboard|词汇 Dashboard]]\n- [[BinnAgentX/Spaced Repetition 使用指南|Spaced Repetition 使用指南]]\n';

const REVIEW_STYLE_SNIPPET_NAME = "binnagentx-spaced-repetition";
const REVIEW_STYLE_SNIPPET = `/* BinnAgentX vocabulary review cards */
...
export default class BinnAgentXLearningSyncPlugin extends Plugin {
  ...
  async initializeLearningLibrary(showNotice = true): Promise<void> {
    ...
    installed += await this.migrateManagedVocabularyTemplate();
  ...
  private async migrateManagedVocabularyTemplate(): Promise<number> {
    const path = `${TEMPLATE_FOLDER}/词汇.md`;
    const file = this.app.vault.getAbstractFileByPath(path);
    if (!(file instanceof TFile)) return 0;
    const content = await this.app.vault.read(file);
    if (
      content !== LEGACY_VOCABULARY_TEMPLATE &&
      content !== BIDIRECTIONAL_VOCABULARY_TEMPLATE &&
      content !== SIMPLE_VOCABULARY_TEMPLATE
    )
      return 0;
    await this.app.vault.modify(file, VOCABULARY_TEMPLATE);
    return 1;
  }
  ...
  private async installReviewStyleSnippet(): Promise<number> {
    ...
    const snippetFolder = `${this.app.vault.configDir}/snippets`;
    const snippetPath = `${snippetFolder}/${REVIEW_STYLE_SNIPPET_NAME}.css`;
    let changed = 0;
    if (!(await adapter.exists(snippetFolder))) {
      await adapter.mkdir(snippetFolder);
    }
    if (!(await adapter.exists(snippetPath))) {
      await adapter.write(snippetPath, REVIEW_STYLE_SNIPPET);
      changed += 1;
    }
    ...
    if (await adapter.exists(appearancePath)) {
      ...
      try {
        ...
        throw new Error(`无法启用 BinnAgentX 闪卡样式：${appearancePath} 不是有效的 JSON`);
      }
    }
    const enabled = arrayStrings(appearance.enabledCssSnippets);
    if (!enabled.includes(REVIEW_STYLE_SNIPPET_NAME)) {
      await adapter.write(
        appearancePath,
        `${JSON.stringify(
          {
            ...appearance,
            enabledCssSnippets: [...enabled, REVIEW_STYLE_SNIPPET_NAME],
          },
          null,
          2,
        )}\n`,
