...
- 两段校准、匹配阅读和微型表达三个任务之间的 `VerticalSliceRun` 编排；
- 基于学习者局部知识状态的材料匹配器；当前内容端口只按任务类型确定性选择首个合格版本；
- 难度反馈、下一任务占位和整条切片的跨任务完成判定；
- Worker 消费 Outbox、模型网关、超时/预算降级和故障注入；
- 桌面阅读工作区、语义标记编辑器、自动保存、差异视图和 IME；
- 真人、教师、内容权利、材料难度和学习效果验证。

下一领域增量应优先实现 `VerticalSliceRun` 跨任务编排和内容端口，再进入桌面标记/输出工作区，避免前端临时逻辑反向定义领域规则。
