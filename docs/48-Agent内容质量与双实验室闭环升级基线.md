...
> 状态：升级设计基线 v0.4；P2、P5 与语法构式首个纵向切片已实施，正式质量路径仍受决策门阻断
> 日期：2026-07-28
...
8. **语法以版本化构式和不可变证据为事实。** 内容、句法候选、挑战、知识整理、
   阅读与表达必须引用同一 `construction_id`；揭示答案、提示纠错、独立识别和
   未验证表达不得折算成同一种掌握证据。
...
语法体系的权威设计、任务拆分、首个纵向切片和完成定义见：

`docs/50-语法知识体系与实施基线.md`

首个工程切片已将 `clause.adverbial.concession.although.v1` 接入：

- 教学目标包和完整材料审核；
- 句法候选与 form/meaning/use 解释；
- 语法挑战及答案揭示、支持纠错、独立纠错的不同回执；
- 知识整理 grammar 候选的目录解析和未知候选 DEFER；
- 构式级不可变证据和 learner state 投影；
- 阅读到表达的同 ID 迁移，其中未验证表达不会提升 productive 掌握；
- 总体语法维度从构式状态汇总，不再把所有 `resolved` 题目视为同质证据。

该结果证明领域契约、持久化和现有学习链路可以闭环，不证明真实解析器准确率、
教研金标质量或表达构式验证效度。G-04、G-05、G-06 与新增 G-08 的正式发布条件
继续阻断生产质量声明。
