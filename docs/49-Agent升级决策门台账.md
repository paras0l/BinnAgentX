...
> 状态：决策进行中 v0.6（P2、P5 与语法构式首个纵向切片工程验收已完成）
> 日期：2026-07-28
...
| G-08 | 语法构式目录与能力证据 | 版本化 Construction 是统一语法 ID；不可变构式证据按 form/meaning/use 与 receptive/productive 投影；总体等级只做派生汇总 | `accepted-for-spike` | `docs/50-语法知识体系与实施基线.md`、迁移 0035 | 首个让步构式已完成目录、内容、挑战、整理、证据和双实验室接线；仍需专家金标、真实解析器、表达构式验证器、正式阈值和延迟真人效度 |
...
6. G-07 开发库盘点：**已完成**；真实环境盘点、兼容窗口和回滚演练
   **待目标环境**；
7. 只有 G-01 至 G-08 的正式发布阻断项全部关闭，才允许把新 Agent 流程标记为正式质量路径。
8. G-08 首个工程切片已完成；只有真实句法金标、表达使用验证、延迟迁移样本和教研
   阈值关闭后，才允许把构式状态称为正式语法能力测量。
...
语法构式首个纵向切片已经证明同一稳定 ID 可以贯穿内容、挑战、整理、学习证据与
表达迁移，并修正了“揭示答案等于掌握”和“完成表达等于正确使用目标语法”的错误
语义；这仍是工程验收，不替代专家金标或真人学习效果。
...
8. G-08 缺真实句法 Provider 的构式映射准确率、专家构式金标、表达侧 form/meaning/use
   验证器和延迟真人迁移数据；当前 productive 状态对未验证表达保持
   `production_unverified`，不得人工改名为已掌握。
...
- v0.6 验收：159 个单元测试、37 个集成测试和 8 个契约测试全部通过；learner
